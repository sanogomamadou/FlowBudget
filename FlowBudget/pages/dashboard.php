...
$currentYear = date('Y'); // récupère l'année actuelle, ex: 2025
// Utiliser l'ID de l'utilisateur pour récupérer les données spécifiques 
$stmt = $pdo->prepare("SELECT * FROM transactions WHERE user_id = ? AND YEAR(date) = ? ORDER BY date DESC");
...
$stmt = $pdo->prepare("SELECT SUM(montant) AS total_expenses FROM transactions WHERE type = 'Dépense' AND user_id =? AND YEAR(date) = ?");$stmt->execute([$user_id, $currentYear]);
